...
/**
 * TableHead — DESIGN.md §4 data-table spec:
 * bg slate-50, 12px bold uppercase tracking-wide text-slate-600,
 * 2px border-bottom, 12–16px padding.
 */
const TableHead = React.forwardRef<HTMLTableCellElement, React.ThHTMLAttributes<HTMLTableCellElement>>(
  ({ className, ...props }, ref) => (
    <th
      ref={ref}
      className={cn(
        'h-10 px-4 text-left align-middle text-[11px] font-bold uppercase tracking-wider text-slate-600 [&:has([role=checkbox])]:pr-0',
        className,
      )}
      {...props}
    />
