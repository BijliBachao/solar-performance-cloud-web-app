...
export async function GET(request: NextRequest) {
  try {
    ...
    // Sorting is owned by the CLIENT (one sort, one place — the page's
    // worst-first ladder + per-column config). The old server-side priority
    // sort was silently discarded by the client's default re-sort anyway
    // (sorting audit, 2026-06-05) — dead logic removed.
