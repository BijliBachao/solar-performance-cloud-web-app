...
export async function GET(request: NextRequest) {
  try {
    ...
    // to a plant assigned to that org (mirrors the admin string-level route's
    // orgPlantIds guard). Without organization_id, SUPER_ADMIN can access any device.
    let plantIdFilter: { in: string[] } | undefined
    if (organizationId) {
      ...
      plantIdFilter = { in: orgPlantIds }
    }

    const device = await prisma.devices.findFirst({
      where: { id: deviceId, ...(plantIdFilter ? { plant_id: plantIdFilter } : {}) },
    ...
    console.error('[Admin Analysis String-Cell]', error)
