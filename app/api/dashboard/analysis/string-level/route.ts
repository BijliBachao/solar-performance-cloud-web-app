...
export async function GET(request: NextRequest) {
  try {
    ...
    // Real nameplate when available; falls back to plant-cap estimate below.
    ...
    const buildRow = (key: string, type: 'active' | 'inactive' | 'unused') => {
      ...
      // Prefer real nameplate from string_configs when admin has configured the string.
      // Falls back to plant_capacity / active_string_count (an estimate).
      const cfg = configByKey.get(key)
      const nameplateKwp = cfg?.panel_count && cfg?.panel_rating_w
        ? Math.round((cfg.panel_count * cfg.panel_rating_w) / 10) / 100  // round to 2 decimals
        : null
