...
export async function GET(request: NextRequest) {
  try {
    ...
    if (userContext.role === 'SUPER_ADMIN') {
      ...
      const assignedPlantIds = assignments.map(a => a.plant_id)
      if (plantId && !assignedPlantIds.includes(plantId)) return NextResponse.json({ error: 'Not found' }, { status: 404 })
      // Non-admin always scoped to assigned plants; empty ⇒ { in: [] } (matches nothing).
      plantWhere = plantScopeWhere(userContext, assignedPlantIds, plantId || null)
    }
